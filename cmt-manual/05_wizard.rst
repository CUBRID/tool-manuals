...
객체별 매핑 옵션(Column, Primary Key, Foreign Key, Index 등)은 5단계에서 트리 구조와 공통 동작만 다루며, 객체 유형별 상세는 :doc:`06_objects`\을 참고한다. 원본 데이터베이스 타입별 연결 필드는 :doc:`07_sourcedb`, 대상 출력 형식별 옵션은 :doc:`08_target`\에서 자세히 다룬다.

.. note::
  [**시작하기**] 버튼은 마지막 단계(6단계 — 최종 확인)에서만 활성화된다. 이전 단계에서는 [**다음 >**]\만 사용할 수 있다.

1단계 — 마이그레이션 유형 선택
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

화면은 두 개의 그룹 — **원본 유형**\과 **대상 유형** — 으로 나뉘며, 각 그룹에서는 하나만 선택할 수 있다.

.. image:: ./cmt_images/image_wizard_1.png

**원본 유형**

CMT가 원본으로 지원하는 데이터베이스 종류는 다음과 같다. 각 항목의 연결 필드와 주의사항은 :doc:`07_sourcedb`\을 참고한다.
...
.. note::
  마지막에 선택한 조합은 사용자별 환경설정에 저장되어 다음 실행 시 동일한 조합으로 시작한다.
...
.. image:: ./cmt_images/image_wizard_2.png
...
.. list-table::
    ...
    * - 필드
      - 비고
    * - **데이터베이스 종류**
      - 1단계에서 선택한 데이터베이스 종류가 기본값. 해당 정보는 변경할 수 없다.
    * - **JDBC 드라이버**
    ...
    * - **데이터베이스 이름**
      - 필수.
    * - **문자 집합**
      - 원본 데이터베이스 종류에 따라 활성화된다(CUBRID, MSSQL 등).
    * - **사용자 이름 / 비밀번호**
      - Username은 필수. Password는 데이터베이스 정책에 따라 선택.
    * - **JDBC 고급 설정...**
      - 사용자 정의 JDBC URL 패턴을 입력할 수 있는 다이얼로그를 연다.

**테스트**

폼 하단의 [**테스트**] 버튼은 입력한 정보로 실제 JDBC 접속을 시도한다. 성공 시 정보 메시지가, 실패 시 오류 메시지와 상세 스택 트레이스를 보여 주는 다이얼로그가 표시된다.
...
파일이 선택되지 않았거나 charset이 비어 있는 경우, 또는 유효하지 않은 mysqldump XML이면 명시적인 오류 메시지가 표시된다.
...
대상이 Online CUBRID인 경우
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

CUBRID 전용 연결 정보 관리 영역과 마이그레이션 옵션 체크박스가 표시된다.

.. image:: ./cmt_images/image_wizard_3_online.png
...
- 대상 CUBRID가 11.2 이상이면 멀티 스키마 모드가 자동으로 활성화된다.
- 대상 버전과 대상 사용자 권한이 저장된다. 이후 객체 매핑 단계 진입 시 11.2 미만 대상이면 Synonym, Grant를 마이그레이션할 수 없다는 경고가 표시되고, 대상 사용자가 DBA 그룹에 속하지 않으면 Grant를 마이그레이션할 수 없다는 경고가 표시된다.
...
.. image:: ./cmt_images/image_wizard_3_offline.png
...
.. list-table::
    ...
    * - **파일 경로**
      - ``<설치경로>/output/``
      - [**찾아보기...**]\로 디렉터리를 선택한다. 끝의 ``/``\는 자동으로 추가된다.
    * - **출력 파일의 Prefix**
      - 원본 데이터베이스 이름
    ...
    * - **LOB files' root path**
      - 비어 있음
      - 대상이 CUBRID dump일 때만 표시. dump 데이터 파일에 기록할 LOB 기준 경로. 비워 두면 출력 경로 아래의 ``lob/<테이블명>/`` 경로가 기록된다. 다른 경로를 지정한 경우 생성된 LOB 파일도 대상 서버에서 그 위치에 맞게 배치한다.
    * - **CUBRID 버전**
      - ``11.2 이상``
      - ``11.2 이상`` 또는 ``11.0 이하`` 중 선택. 멀티 스키마 출력 구조를 결정한다.
    * - **스키마 파일 분리**
      - ON
...
대상 출력 형식별 디렉터리 구조와 파일 명명 규약은 :doc:`08_target`\을 참고한다.
...
.. image:: ./cmt_images/image_wizard_4.png

**화면 구성**

- 상단: [**데이터베이스 오브젝트 업데이트**] 버튼 — 원본 데이터베이스에서 객체들을 조회한다. 만약, 기존에 이미 캐시된 객체들이 있다면 초기화 후 다시 조회를 한다.
...
.. list-table::
    ...
    * - 대상 버전
      - 동작
    * - 11.2 이상
      - 멀티 스키마가 자동으로 활성화된다. 원본 스키마별로 대상 스키마를 별도로 지정할 수 있다.
    * - 11.0 이하
      - 모든 객체가 단일 사용자 스키마로 통합된다. 서로 다른 원본 스키마에 동일한 이름의 객체가 존재하면 다음 단계 진입 시 충돌 안내 다이얼로그가 표시된다.

.. note::
  대상이 파일인 경우 본 단계에서 별도 화면이 표시되지 않고, 3단계의 **CUBRID 버전** 선택이 동일한 역할을 한다.
...
.. image:: ./cmt_images/image_wizard_5.png
...
.. list-table::
    ...
    * - 개별 Table (또는 그 하위 PK / FKs / Indexes 노드)
      - 탭으로 컬럼, PK, FK, 인덱스를 편집한다.
    * - 단일 Column
      - 이름, 데이터 타입, 길이, 정밀도, scale, 기본값, NULL 허용 여부를 편집한다.
    * - 단일 Index / Foreign Key / Serial(Sequence) / View / Synonym
      - 각각의 속성에 맞는 편집 화면이 표시된다.
    * - 단일 View / Procedure / Function
      - 대상으로 생성될 SQL을 편집한다.
    * - 단일 SQL Table
...
- 여러 원본 스키마에 동일한 이름의 객체가 존재하고 대상이 단일 스키마일 때 — **중복된 오브젝트 이름 변경** 안내
- 원본에 LOB Column이 있을 때 — **LOB Information** 안내
- 원본이 CUBRID가 아닌 Online 데이터베이스일 때 — 식별자가 모두 소문자로 변환된다는 경고
- 원본과 대상의 char / varchar 길이 차이가 변환을 요구할 때 — char Column 길이 조정 다이얼로그
- 대상이 온라인이고, 11.0 이하 버전인 경우 Synonym 마이그레이션 미지원 경고
- 대상이 온라인이고, 11.0 이하 또는 DBA 권한이 없는 경우 Grant 마이그레이션 미지원 경고
...
.. image:: ./cmt_images/image_wizard_6.png
...
- **원본 데이터베이스** — 원본 유형, 호스트 IP, 데이터베이스 종류와 이름, 포트, 사용자, Charset, Timezone. XML dump의 경우 파일 경로와 Charset, Timezone.
- **대상 데이터베이스** — 대상 유형. 파일 대상의 경우 출력 경로, 멀티 스키마 사용 여부, 객체 카테고리별(Table / View / PK / FK / Serial / Synonym / Grant / Procedure / Function / Unique Index / Index / Data / UPDATE STATISTICS) 출력 경로가 파란색으로 강조되어 표시된다.
- **추출될 테이블 / 추출될 뷰 / 추출될 SQL 데이터 / 추출될 외래키 / 추출될 인덱스 / 추출될 시리얼** — 원본과 대상의 매핑 목록.
- **추출될 시노님 / 추출될 권한** — 대상 CUBRID 11.2 이상일 때만 표시된다.

성능 설정 다이얼로그
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

하단 툴바의 [**고급 성능 설정**]\을 누르면 성능 설정 다이얼로그가 열린다.
...
.. list-table::
    ...
    * - **파일별 최대 레코드 개수**
      - 0 이상 (XLS는 상한 적용)
      - 0
      - 대상이 파일이고 **테이블별 파일 분할 출력**\이 켜져 있을 때만
    * - **테이블별 진행률 생략**
      - 체크박스
      - 미체크
      - 체크 시 안내 메시지 표시

[**Restore Defaults**] 버튼은 모든 값을 위 기본값으로 되돌린다.

추출 스레드(Export thread count)는 원본 데이터베이스에서 데이터를 동시에 읽어 오는 Table의 개수, 적재 스레드(Import Thread Count Per Table)는 한 Table에 동시에 데이터를 대상으로 적재하는 작업 수이다.
...
.. image:: ./cmt_images/image_wizard_6_ddl.png
...
체크박스 [**Save Source Catalog**]\을 켜면 원본 데이터베이스의 카탈로그가 스크립트 XML에 함께 저장된다. 다음에 동일한 스크립트로 다시 실행할 때 원본 데이터베이스에 접속하지 않고도 객체 매핑을 복원할 수 있다.

실행 또는 예약
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

[**시작하기**]\을 누르면 다음 순서로 동작한다.
...
- **마이그레이션명** — 현재 설정 이름이 채워져 있다. 워크스페이스 내에서 유일한 이름이어야 한다.
- [**바로 시작하기**] — 즉시 마이그레이션을 시작하고 진행 화면을 연다.
- [**예약하기**] — 스크립트를 저장한 뒤 예약 다이얼로그를 연다.
- [**취소**] — 닫고 6단계로 돌아간다.

[**예약하기**]\을 누르면 예약 다이얼로그가 열린다. 다음 세 가지 모드 중 하나를 선택한다.
...
.. note::
  예약된 마이그레이션은 실행 시점에 CMT가 켜져 있어야 동작한다. 워크벤치를 상시 켜둘 수 없는 환경에서는 콘솔 모드(``migration.sh start`` / ``migration.bat start``)와 운영체제의 cron / Task Scheduler 조합을 권장한다. 자세한 내용은 :doc:`09_console`\을 참고한다.

마이그레이션 실행 및 모니터링
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

[**바로 시작하기**]\을 누르면 에디터 영역에 진행 화면이 열리고 실제 마이그레이션이 시작된다.

.. image:: ./cmt_images/image_wizard_progress.png
...
.. list-table::
    ...
    * - **진행률(%)**
      - 진행률 백분율.
    * - **소유자**
      - 스키마 소유자. 원본이 멀티 스키마를 지원하지 않으면 표시되지 않는다.
...
- 마이그레이션이 정상적으로 종료되면 에디터 탭의 아이콘이 진행 표시에서 녹색 체크 표시로 바뀐다.
...
본 챕터에서는 마이그레이션 마법사의 모든 단계와 옵션을 다루었다. 좀 더 자세한 내용은 다음 챕터를 참고한다.
