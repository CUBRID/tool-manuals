...
[**기본값**] 버튼은 모든 값을 환경설정의 **마이그레이션** 페이지에 설정된 값(기본 상태에서는 위 기본값)으로 되돌린다.
...
.. note::
  예약된 마이그레이션은 실행 시점에 CMT가 켜져 있어야 동작한다. CMT를 상시 켜둘 수 없는 환경에서는 콘솔 모드(``migration.sh start`` / ``migration.bat start``)와 운영체제의 cron / Task Scheduler 조합을 권장한다. 자세한 내용은 :doc:`09_console`\를 참고한다.
