...
객체별 매핑 옵션(Column, Primary Key, Foreign Key, Index 등)은 5단계에서 트리 구조와 공통 동작만 다루며, 객체 유형별 상세는 :doc:`06_objects`\를 참고한다. 원본 데이터베이스 타입별 연결 필드는 :doc:`07_sourcedb`, 대상 출력 형식별 옵션은 :doc:`08_target`\에서 자세히 다룬다.
...
.. note::
  마지막에 선택한 조합은 저장되어 다음 실행 시 동일한 조합으로 시작한다.
...
.. list-table::
    ...
    * - **사용자 이름 / 비밀번호**
      - 사용자 이름은 필수이며, 비밀번호는 데이터베이스 정책에 따라 선택이다.
    * - **JDBC 고급 설정...**
      - 사용자 정의 JDBC URL 패턴을 입력할 수 있는 다이얼로그를 연다.

**테스트**

폼 하단의 [**테스트**] 버튼은 입력한 정보로 실제 JDBC 접속을 시도한다. 성공 시 정보 메시지가, 실패 시 오류 원인의 상세 메시지를 보여 주는 다이얼로그가 표시된다.
...
대상 출력 형식별 디렉터리 구조와 파일 명명 규약은 :doc:`08_target`\를 참고한다.
...
[**기본값**] 버튼은 모든 값을 위 기본값으로 되돌린다.

**추출 스레드 개수**\는 원본에서 동시에 데이터를 추출하는 Table 수, **테이블당 입력 스레드 개수**\는 한 Table을 동시에 적재하는 작업 수이다.
...
.. note::
  예약된 마이그레이션은 실행 시점에 CMT가 켜져 있어야 동작한다. 워크벤치를 상시 켜둘 수 없는 환경에서는 콘솔 모드(``migration.sh start`` / ``migration.bat start``)와 운영체제의 cron / Task Scheduler 조합을 권장한다. 자세한 내용은 :doc:`09_console`\를 참고한다.

마이그레이션 실행 및 모니터링
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

[**바로 시작하기**]\를 누르면 에디터 영역에 진행 화면이 열리고 실제 마이그레이션이 시작된다.
