...
파일이 선택되지 않았거나 문자집합이 비어 있는 경우, 또는 유효하지 않은 mysqldump XML이면 명시적인 오류 메시지가 표시된다.
...
- **원본 데이터베이스** — 원본 유형, 호스트 IP, 데이터베이스 종류와 이름, 포트, 사용자, 문자집합, 시간대. XML dump의 경우 파일 경로와 문자집합, 시간대.
