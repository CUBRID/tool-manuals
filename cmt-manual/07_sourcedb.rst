...
연결 다이얼로그의 **문자 집합** 항목은 CUBRID·MSSQL 연결에서만 활성화되며 기본값은 ``UTF-8``\이다. 그 외 DB에서는 이 항목이 비활성화된다. 원본 DB의 캐릭터셋과 클라이언트 문자셋이 다르면 문자 변환 과정에서 데이터 손실이 발생할 수 있으므로, 원본 DB의 캐릭터셋과 일치시키는 것을 권장한다.
...
비활성 상태(``DISABLED``)의 Primary Key·Foreign Key 제약 조건도 마이그레이션 대상에서 제외된다. 비활성 제약 조건은 Oracle에서도 DML 검증에 참여하지 않으므로, 이 동작과 일치시킨 것이다. 비활성 제약 조건도 옮기려면 원본 Oracle에서 미리 활성화한 뒤 마이그레이션을 수행한다.
