...
상단 메뉴 ``마이그레이션 > 기본 설정``\를 선택하면 환경 설정 다이얼로그가 열린다.

.. image:: ./cmt_images/image_config_1.png
...
.. image:: ./cmt_images/image_config_2.png
...
.. list-table::
    ...
    * - **윈도우 최대 크기로 보기**
      ...
      - OFF
    * - **종료시 묻지 않고 종료하기**
      - 종료 시 "Are you sure to exit CUBRID Migration Toolkit?" 확인 다이얼로그를 표시하지 않는다. 종료 확인 다이얼로그의 토글을 통해서도 변경 가능.
      - OFF
...
마이그레이션 작업의 기본 성능 값과 임시 디렉토리를 설정한다. 여기서 정한 값은 새 마법사를 시작할 때의 초기값이 되며, 작업별로는 마법사의 **고급 성능 설정**\로 덮어쓸 수 있다.

.. image:: ./cmt_images/image_config_3.png
...
.. list-table::
    ...
    * - **기본 추출(Export) 스레드 개수**
      ...
      - 4
    * - **테이블당 기본 입력(Import) 스레드 개수**
      - 한 테이블을 대상 DB로 적재할 때 사용할 병렬 스레드 수.
      - 3
    * - **페이지 패치 개수 기본값**
...
값별 권장 조정 방향은 :doc:`12_advanced`\을 참고한다.

**기타 설정**
""""""""""""""""""""""""""""""""""""""""""""""""

- **템플릿 파일 경로** — CMT가 마이그레이션 도중 사용하는 임시 파일이 저장되는 디렉토리. **찾아보기...**\로 디렉토리 다이얼로그를 열어 변경한다. 디스크 여유 공간이 부족하거나 임시 파일을 별도 저장 공간에 두고 싶을 때 변경한다.
...
.. image:: ./cmt_images/image_config_4.png

매핑은 별도의 추가/편집/삭제 버튼이나 편집 다이얼로그 없이 표에서 직접 셀을 인라인 편집하는 방식이다. 대상 컬럼(대상 데이터 타입·전체 자릿수·소수점 이하 자릿수)만 수정할 수 있으며, 원본 컬럼은 편집되지 않는다.
...
MySQL과 MariaDB 매핑 페이지 아래에는 별도의 **기타 설정** 하위 페이지가 있다. 원본의 비표준 날짜/시간 값과 ``CHAR(0)`` 처리 방식을 지정한다.
...
.. list-table::
    ...
    * - 설정
      - 설명
    * - **UNPARSED_TIME**
      - 파싱 불가한 ``TIME`` 값을 어떻게 처리할지(예: NULL 치환, 에러 기록).
    * - **UNPARSED_DATE**
      - 파싱 불가한 ``DATE`` 값(``0000-00-00`` 등)을 어떻게 처리할지.
    * - **UNPARSED_TIMESTAMP**
      - 파싱 불가한 ``TIMESTAMP`` 값을 어떻게 처리할지.
    * - **REPLAXE_CHAR0**
      - ``CHAR(0)`` 타입을 대상에서 어떤 타입으로 변환할지(CUBRID에는 ``CHAR(0)``\이 없다).
...
.. image:: ./cmt_images/image_config_5.png
...
기본 워크스페이스 경로는 CMT 설치 디렉토리 바로 아래의 ``<설치 경로>/workspace/cmt/``\이다. OS별 차이는 없으며, CMT가 시작 시 직접 이 경로를 설정한다.

CMT 12.0은 워크스페이스 위치를 런타임에 변경하지 않는다. Eclipse 표준 ``-data <경로>`` 인자는 적용되지 않으므로 워크스페이스 위치를 분리하려면 CMT를 별도 디렉토리에 추가 설치한다.
...
.. code-block:: text
    ...
    │       ├── history/         # 마이그레이션 히스토리 메타데이터
    │       ├── schemacache/     # 원본 스키마 캐시
    │       ├── errors/          # 적재 실패 레코드(타임스탬프 하위 디렉토리)
    │       └── temp/            # 임시 파일(설정으로 변경 가능)
    └── jdbc/                    # 번들 JDBC 드라이버(워크스페이스 아님)

``jdbc/``\는 워크스페이스가 아니라 설치 디렉토리 바로 아래에 위치한다. ``log/``, ``report/``, ``script/``, ``history/``, ``schemacache/``, ``errors/``\는 CMT가 직접 관리하므로 수동으로 편집하지 않는 것을 권장한다. 임시 파일 디렉토리는 **마이그레이션** 페이지의 **템플릿 파일 경로** 설정으로 변경할 수 있다.

관련 챕터
^^^^^^^^^

- 성능 항목 권장값과 메모리 조정 — :doc:`12_advanced`
