...
- **마이그레이션** — 마이그레이션 기본 성능 값과 임시 디렉터리
...
마이그레이션 작업의 기본 성능 값과 임시 디렉터리를 설정한다. 여기서 정한 값은 새 마법사를 시작할 때의 초기값이 되며, 작업별로는 마법사의 **고급 성능 설정**\으로 덮어쓸 수 있다.
...
- **임시 파일 경로** — CMT가 마이그레이션 도중 사용하는 임시 파일이 저장되는 디렉터리. **찾아보기...**\로 디렉터리 다이얼로그를 열어 변경한다. 디스크 여유 공간이 부족하거나 임시 파일을 별도 저장 공간에 두고 싶을 때 변경한다.
...
매핑은 별도의 추가/편집/삭제 버튼이나 편집 다이얼로그 없이 표에서 직접 셀을 인라인 편집하는 방식이다. 대상 칼럼(대상 데이터 타입·전체 자릿수·소수점 이하 자릿수)만 수정할 수 있으며, 원본 칼럼은 수정할 수 없다.
...
표의 칼럼 헤더를 클릭하면 해당 칼럼 기준으로 정렬된다.
...
CMT 설치 디렉터리에는 다음 CUBRID JDBC 드라이버 12종이 함께 제공된다.
...
번들 드라이버는 ``<설치 경로>/jdbc/`` 디렉터리에 위치한다. 그 외 버전의 CUBRID JDBC 드라이버가 필요하면 **Update CUBRID Drivers** 버튼으로 cubrid.org에서 최신 드라이버를 내려받거나, **추가** 버튼으로 직접 JAR 파일을 등록한다.
...
워크스페이스는 CMT가 사용자별 설정과 마이그레이션 산출물을 모아 두는 디렉터리이다. 한 워크스페이스 안에 연결 정보, 스크립트, 이력, 보고서, 환경 설정이 함께 들어 있다.

위치
""""""""""""""""""""""""""""""""""""""""""""""""

기본 워크스페이스 경로는 ``<설치 경로>/workspace/``\이며, CMT 산출물은 그 아래 ``cmt/`` 디렉터리에 저장된다. OS별 차이는 없으며, CMT가 시작 시 직접 이 경로를 설정한다.
...
.. code-block:: text
    ...
    │       ├── errors/          # 적재 실패 레코드(타임스탬프 하위 디렉터리)
    │       └── temp/            # 임시 파일(설정으로 변경 가능)
    └── jdbc/                    # 번들 JDBC 드라이버(워크스페이스 아님)

``jdbc/``\는 워크스페이스가 아니라 설치 디렉터리 바로 아래에 위치한다. ``log/``, ``report/``, ``script/``, ``history/``, ``schemacache/``, ``errors/``\는 CMT가 직접 관리하므로 수동으로 편집하지 않는 것을 권장한다. 임시 파일 디렉터리는 **마이그레이션** 페이지의 **임시 파일 경로** 설정으로 변경할 수 있다.
