...
객체 매핑 화면은 원본 데이터베이스에서 추출한 객체 카탈로그를 사용자가 검토하고 대상 CUBRID에 맞게 조정하는 작업 공간이다. 객체 종류에 따라 노출되는 옵션이 다르므로 본 챕터는 종류별 안내 형식으로 정리한다. 각 절은 객체 개요, 노출되는 필드/옵션, 그리고 마이그레이션 결과에 영향을 주는 주의사항으로 구성된다.
