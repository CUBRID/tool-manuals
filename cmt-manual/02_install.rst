...
2. 압축이 풀린 디렉터리의 ``cubridmigration.exe`` 파일을 실행한다.
...
1. 다운로드한 ``.tar.gz`` 파일을 원하는 경로에 압축 해제한다.
   ...
   .. code-block:: bash
      ...
      tar -xzf CUBRID-Migration-Toolkit-<버전>-linux-x86_64.tar.gz -C <설치할 디렉터리>

   ``-C``\는 압축을 해제할 대상 디렉터리를 지정하는 ``tar`` 옵션이다. 설치 경로는 사용자 계정에 쓰기 권한이 있는 디렉터리로 지정한다.

2. 압축이 풀린 디렉터리로 이동해 실행 파일을 실행한다.
...
수동으로 업그레이드하는 경우, 기존 설치 디렉터리는 그대로 두고 새 ``.zip`` 또는 ``.tar.gz``\를 동일한 경로에 풀어 덮어쓴다.

.. warning::
  수동 업그레이드 시 연결 정보 등이 포함된 **워크스페이스 디렉터리를 반드시 백업**\한다. CMT의 기본 워크스페이스는 설치 디렉터리 아래의 ``workspace/cmt/``\이므로, 이 디렉터리를 복사해 둔다.

삭제
^^^^^^

CMT를 완전히 삭제하려면 설치 디렉터리를 직접 삭제한다. 기본 워크스페이스는 설치 디렉터리 하위의 ``workspace/cmt/``\에 있으므로 설치 디렉터리를 삭제하면 함께 제거된다.

Windows
""""""""""

1. 연결 정보와 이력을 보존해야 한다면 ``workspace\cmt\`` 디렉터리를 별도로 백업한다.
2. 파일 탐색기에서 CMT 설치 디렉터리(예: ``C:\CUBRID\cubridmigration``)를 삭제한다.
