...
행을 더블 클릭하면 **상세** 탭으로 이동한다. 맨 아래의 **record** 행에서는 **이전 통계** 서브탭, 그 외 객체 종류에서는 **DB 객체** 서브탭이 열린다.
...
**설정 정보** 탭은 **모든 테이블을 엑셀 파일로 저장** 저장 대상이 아니다. 설정을 그대로 다시 활용하려면 **마이그레이션 마법사 열기** 버튼을 사용해 마법사를 다시 열거나, :doc:`05_wizard`\의 **스크립트 내보내기** 기능으로 내보낸다.
...
적재 실패 행은 대상 연결의 ``write_error_records`` 옵션이 켜져 있을 때만 기록된다. 한 번의 마이그레이션에 대해 ``errors/<밀리초epoch>/`` 디렉토리가 만들어지고, 그 아래에 원본 Table마다 ``<원본 테이블명>.sql`` 파일이 생성된다. ``<밀리초epoch>``\는 마이그레이션 시작 시각의 밀리초 단위 값이다. 이 파일들을 수정한 뒤 ``csql``\로 다시 적용하면 실패한 행만 보충 적재할 수 있다.
