...
본 챕터는 마이그레이션이 끝난 뒤 결과를 확인하는 방법을 다룬다. CMT는 모든 마이그레이션에 대해 보고서를 자동으로 생성한다. GUI에서는 탭으로 구성된 보고서 화면, 콘솔에서는 ``report`` 명령으로 동일한 내용을 확인할 수 있다. 보고서 / 로그 / 산출 파일이 워크스페이스의 어느 디렉터리에 저장되는지도 함께 정리한다.
...
상단에는 툴바, 본문에는 6개의 탭이 왼쪽에서 오른쪽 순서로 배치된다.
...
.. list-table::
    ...
    * - 버튼
      - 동작
    * - **모든 테이블을 엑셀 파일로 저장**
      - 디렉터리 선택 다이얼로그를 열고, 시작 시각 타임스탬프를 공통 접두사로 하는 4개 파일을 저장한다 (상세는 "보고서 저장" 항목)
    * - **마이그레이션 재실행...** (툴팁: **마이그레이션 마법사 열기**)
      - 동일한 설정으로 마법사를 다시 연다. 1단계부터 모든 입력이 채워진 상태에서 시작되므로, 객체 선택이나 매핑만 일부 조정해 재실행할 수 있다
    * - **파일 저장 폴더 열기**
      - 파일 출력 모드일 때만 표시된다. OS 파일 탐색기로 출력 디렉터리를 연다
...
.. list-table::
    ...
    * - 탭
      - 설명
    * - **요약**
      - 시작 / 종료 시각, 총 소요 시간, 출력 디렉터리, 객체 종류별 개수와 진행률 요약
    * - **상세**
...
- **출력 위치** — 파일 출력 모드(CUBRID dump / SQL / CSV / XLS)일 때만 표시된다. 경로를 클릭하면 OS 파일 탐색기로 디렉터리가 열린다

그 아래의 표는 객체 종류별 결과를 5개 열로 보여 준다.
...
행을 더블클릭하면 **DDL**\과 **오류** Column의 긴 텍스트가 별도 다이얼로그에 표시되어 한 화면에서 확인할 수 있다.

상세 — 이전 통계
""""""""""""""""""""""""""""""""""""""""""""""""

Table별 데이터 적재 결과를 9개 열로 보여 준다.
...
행이 하나도 없는 Table은 개수가 ``0``\으로 표시된다.

변경된 객체
""""""""""""""""""""""""""""""""""""""""""""""""

마이그레이션 과정에서 이름이 바뀐 객체를 **유형 / 원본 객체 / 대상 객체** 3개 열로 보여 준다. 원본과 다른 이름으로 마이그레이션된 객체를 한눈에 확인하는 용도이다.
...
마이그레이션 실행 중 기록된 전체 로그가 텍스트로 표시된다. 시간 순서대로 단계별 진행 메시지, 경고, 오류가 모두 포함된다. 툴바의 **모든 테이블을 엑셀 파일로 저장** 버튼으로 외부 파일에 보존할 수 있다 (저장 형식은 아래 "보고서 저장" 항목 참고).
...
툴바의 **모든 테이블을 엑셀 파일로 저장** 버튼을 누르면, 선택한 디렉터리에 마이그레이션 시작 시각(``yyyy_MM_dd_HH_mm_ss_SSS``)을 공통 접두사로 하는 4개 파일이 저장된다.
...
- :doc:`08_target` — 대상 유형별 출력 디렉터리 구조
