...
1. **스키마 생성** — 대상이 멀티 스키마이고 ``user schema 추가`` 옵션이 켜져 있으면 대상에 없는 사용자 스키마를 ``CREATE USER``\로 먼저 생성한다.
...
Online CUBRID 대상에서는 사용자가 대상 버전을 선택하지 않는다. 마법사가 접속한 CUBRID 서버의 버전을 자동으로 감지해 대상 버전으로 설정하며, 별도의 버전 선택 UI는 표시되지 않는다.
...
데이터 파일도 별도 디렉토리로 분리되지 않고 같은 ``<원본 스키마>/`` 디렉토리 아래에 들어간다. 단, **테이블별 파일 분할 출력** (``one_table_one_file``) 옵션을 켜면 데이터 파일만 한 단계 더 들어간 ``<원본 스키마>/objects/`` 아래에 생성된다. CSV 와 XLS 대상은 이 옵션 값과 무관하게 출력 동작상 항상 Table별 파일을 사용하므로 데이터 파일이 ``<원본 스키마>/objects/`` 아래에 놓인다.
...
- ``.xls`` 파일 하나가 Table 하나에 대응하며, 시트는 항상 하나이다. 파일당 행 수 제한과 분할 조건은 아래 "행 수 제한"을 참고한다.
...
XLS 한 파일(시트 1개)은 형식 한계상 65,536행을 넘을 수 없다. 한 Table을 여러 XLS 파일로 나누려면 **파일별 최대 레코드 개수**\를 0보다 큰 값으로 지정한다(XLS 대상에서는 65,536 이하로 제한됨). 지정하지 않으면(기본값 0) 파일이 분할되지 않으며, 65,536행을 넘는 데이터는 기록되지 않고 오류로 보고된다.
