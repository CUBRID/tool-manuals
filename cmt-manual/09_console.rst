...
.. list-table::
    ...
    * - JVM 최대 힙
      - ``-Xmx4096M`` (4 GB)
      - 대용량 DB 처리 시 ``migration.sh`` / ``migration.bat``\을 수정해 상향
    * - JRE 위치
...
.. tip::
  ``-Xmx`` 값으로 최대 힙 크기를 조정할 수 있으며, GUI 모드의 힙 조정은 :doc:`12_advanced`\(고급 사용)를 참고한다.
...
.. code-block:: bash
  ...
  ./migration.sh start -sd /opt/drivers/ojdbc8.jar \
...
.. list-table::
    ...
    * - ``-ps``
      - 정수
      - ``50``
      - 한 페이지에 출력할 라인 수. 유효한 정수가 아니면 기본값으로 폴백.
...
.. code-block:: properties
  ...
  cubrid_prod.driver=/opt/drivers/cubrid-jdbc.jar
...
- **경로**: ``<install>/workspace/cmt/log/cubrid-migration.log``
