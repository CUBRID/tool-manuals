...
.. list-table::
    ...
    * - JRE 위치
      ...
      - **현재 작업 디렉터리(cwd) 기준**\으로 해석된다.
    * - ``JAVA_HOME``
      - 사용하지 않음
      - 런처가 동봉된 JRE를 강제 사용
    * - 작업 디렉터리
      - 호출 시점의 현재 디렉터리(명령을 실행한 위치)
      - XML 스크립트와 ``-tp`` 경로도 이 디렉터리 기준으로 해석됨

.. tip::
  ``-Xmx`` 값으로 최대 힙 크기를 조정할 수 있으며, GUI 모드의 힙 조정은 :doc:`12_advanced`\을 참고한다.

.. important::
  ``migration.sh`` / ``migration.bat``\은 현재 작업 디렉터리(``$PWD``)를 기준으로 동봉된 JRE와 실행 파일을 찾는다. 따라서 **CMT 설치 디렉터리로 이동한 뒤 실행**\해야 하며, 다른 위치에서 절대 경로로 호출하면 JRE를 찾지 못해 실패한다.
...
.. list-table::
    ...
    * - ``-tp``
      - 디렉터리 경로
      - 스크립트 값
      - 파일 기반 대상으로 출력할 때 산출물이 저장될 디렉터리
    * - ``-xml``
      - 파일 경로
      - (없음)
...
스크립트에 지정된 출력 경로 대신 ``-tp`` 디렉터리에 산출물을 저장한다.
...
.. list-table::
    ...
    * - ``-l``
      - 없음
      - 보고서 디렉터리에서 가장 최근에 생성된 ``.mh`` 파일을 자동으로 선택
    * - ``-ao``
      - 없음
...
사용자는 자신의 이력 디렉터리에 실제로 존재하는 파일 이름으로 대체해 입력한다.
...
``1716166534123.mh`` 자리에는 실제 이력 디렉터리에 존재하는 파일 이름을 입력한다.
...
- **위치**: CMT 설치 디렉터리 루트 (``migration.sh`` / ``migration.bat``\와 같은 디렉터리)
...
콘솔 출력과는 별개로, 실행 로그는 항상 CMT 로그 디렉터리에 기록된다.
