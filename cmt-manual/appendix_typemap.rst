...
본 부록은 CMT가 7개 원본 데이터베이스의 칼럼 데이터 타입을 CUBRID 데이터 타입으로 변환할 때 사용하는 **기본 매핑**\을 한 곳에 정리한 레퍼런스이다. 본문 :doc:`07_sourcedb`\에서 각 원본 DB의 특이 사항을 다루며, 매핑 전체표가 필요한 경우 본 부록을 참고한다.
