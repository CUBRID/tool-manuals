...
.. image:: ./cmt_images/image_quickstart_1.png
  :alt: 새 마이그레이션 버튼 클릭
...
.. image:: ./cmt_images/image_quickstart_2.png
  :alt: 마이그레이션 유형 선택
...
원본 Oracle 사용자 (스키마)와 대상 CUBRID 사용자를 매핑한다. 원본 측 **원본 스키마** 열에서 이관할 스키마에 체크하고, **대상 스키마** 열에서 대상 CUBRID에서 사용할 스키마 이름을 지정한다.

.. image:: ./cmt_images/image_quickstart_5.png
  :alt: 스키마 매핑
...
.. image:: ./cmt_images/image_quickstart_6.png
  :alt: 객체 매핑

G. 마이그레이션 전 최종 확인 (6단계)
"""""""""""""""""""""""""""""""""""""""

6단계에서는 지금까지 설정한 내용을 한 화면에 모아 보여준다. **DDL 미리보기** 버튼으로 생성될 DDL을 미리볼 수 있다.
...
**바로 시작하기**\를 누르면 마이그레이션 진행 화면이 에디터 영역에 열린다. 상단 진행 표에서 Table별 처리 행 수와 진행률을 실시간으로 확인할 수 있다. 하단 로그 패널에는 현재 처리 중인 객체와 메시지가 출력된다.

진행 중 **중단** 버튼으로 작업을 중단할 수 있다. 작업이 진행되는 동안에는 진행 화면 탭을 닫을 수 없다.

.. image:: ./cmt_images/image_quickstart_8.png
  :alt: 마이그레이션 진행

I. 마이그레이션 보고서 확인
""""""""""""""""""""""""""""""

마이그레이션이 끝나면 보고서를 바로 열지 묻는 다이얼로그가 표시된다. 보고서에는 객체 종류별 성공/실패 건수, 마이그레이션된 레코드 수, 에러 메시지 등이 정리되어 있다.

보고서는 워크스페이스에 보관되므로 이후 메인 화면의 **마이그레이션 이력**\에서 다시 열어볼 수 있다. 자세한 내용은 :doc:`10_report`\을 참고한다.

.. image:: ./cmt_images/image_quickstart_9.png
  :alt: 마이그레이션 보고서
...
마법사의 1단계와 3단계가 다를 뿐, 나머지 흐름은 시나리오 1과 같다.
...
.. image:: ./cmt_images/image_quickstart_10.png
  :alt: 새 마이그레이션 시작
...
.. image:: ./cmt_images/image_quickstart_12.png
  :alt: 원본 Oracle 연결 정보

D. 출력 파일 설정 (3단계)
""""""""""""""""""""""""""""

대상 데이터베이스 선택 대신 출력 파일 설정 화면이 나타난다. 다음 항목을 지정한다.

- **파일 경로**: dump 파일을 저장할 디렉토리
- **출력 파일의 Prefix**: 생성될 파일 이름 앞에 붙는 문자열. 예를 들어 Prefix가 ``oracle`` 이고 원본 스키마가 ``HR`` 이면 ``oracle_HR_class``, ``oracle_HR_object`` 같은 이름으로 파일이 생성된다. 기본값은 DB명이 된다.
- **CUBRID 버전**: ``11.2 이상`` 또는 ``11.0 이하`` — 적재할 대상의 CUBRID 버전에 맞게 선택
- **문자 집합**: ``UTF-8``
- **시간대**: 타임존 타입 (``DATETIMETZ`` 등)을 사용하는 경우 지정
- **스키마 파일 분리**: 스키마 DDL을 Table, PK, FK, Index 등 객체 종류별 파일로 분리할지 여부
- **LOB files' root path**: dump 데이터 파일에 기록할 BLOB/CLOB 파일의 기준 경로. 비워 두면 CMT가 출력 경로 아래의 ``lob/<테이블명>/`` 경로를 기록한다. 대상 서버의 다른 경로에서 ``loaddb``\를 실행해야 한다면 해당 경로를 지정하고, 생성된 LOB 파일도 그 위치에 맞게 배치한다.

.. image:: ./cmt_images/image_quickstart_13.png
  :alt: 출력 파일 설정
...
.. image:: ./cmt_images/image_quickstart_14.png
  :alt: 스키마 매핑 (오프라인)
...
.. image:: ./cmt_images/image_quickstart_15.png
  :alt: 객체 매핑 (오프라인)
...
.. image:: ./cmt_images/image_quickstart_16.png
  :alt: 최종 확인 (오프라인)
...
.. image:: ./cmt_images/image_quickstart_17.png
  :alt: 마이그레이션 진행 (오프라인)

I. 마이그레이션 보고서 확인
""""""""""""""""""""""""""""""

마이그레이션이 끝나면 보고서가 열린다. 보고서의 **요약** 탭에서 **출력 위치**\를 클릭하면 OS의 파일 탐색기로 출력 폴더가 열린다.
