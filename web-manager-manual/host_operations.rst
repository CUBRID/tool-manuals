...
:doc:`host` 의 호스트 목록에서 실행할 수 있는 개별 작업(추가/수정/삭제, 그룹 관리, CMS 사용자 관리 등)의
화면별 사용법을 정리한다.

.. image:: /images/host-context-menu.png
   :width: 480px

호스트를 우클릭하면 위와 같은 메뉴가 열린다.

.. image:: /images/host-group-context-menu.png
   :width: 480px

그룹을 우클릭하면 위와 같은 그룹 전용 메뉴가 열린다.
...
툴바 **+ 추가** 를 클릭하면 **새 연결(New Connection)** 대화창이 열린다. 대화창의 섹션 구성은 다음과 같다.
...
하단 버튼: **취소(Cancel)** / **변경 저장(Save Changes)** (로그인 없이 저장만) / **연결 및 저장(Connect & Save)** (연결 확인 후 즉시 로그인까지).

* 주소:포트:사용자 이름(Username)이 모두 같은 연결은 중복 등록할 수 없다 — 같은 주소:포트라도 사용자 이름이 다르면
  별개로 등록할 수 있다.
* Alias(별칭)는 주소/포트/사용자 이름과 무관하게, 등록된 모든 호스트를 통틀어 중복될 수 없다.

.. note::

    **저장(Save Changes)** 을 누르는 시점에는 호스트에 아무 요청도 가지 않는다 — 입력한 값이 그대로 저장될 뿐이다.
    실제로 호스트에 접속을 시도하는 것은 **연결 및 저장** 을 눌렀을 때뿐이다.
...
.. warning::
    ...
    **연결 및 저장(Connect & Save)** 는 로그인 시도 *전에* 새 비밀번호를 먼저 저장한다. 즉 비밀번호를 잘못 입력해서 로그인이 실패해도
...
툴바 **새 그룹(New Group)** 또는 그룹 우클릭 → **그룹 이름 변경(Rename Group)** 을 선택한다. 필드는 **그룹 이름(Group Name)** \* 하나이다.
...
.. note::
    ...
    **그룹 이름(Group Name)** 은 문자나 숫자로 시작해야 한다 — 언더바(``_``)나 다른 기호로 시작하는 이름은
    입력할 수 없다(예: ``_foo``, ``#group``). 그 외에는 길이 제한도, 다른 그룹과의 중복 검사도 없다 — 같은
    이름을 가진 그룹이 여러 개 있어도 된다.
...
필드: New Password(새 비밀번호) \*, Verify New Password(비밀번호 확인) \*.
호스트의 CMS 관리자 비밀번호 자체를 바꾸는 것이며, NCA 서버에 저장된 프로필에 변경된 비밀번호가 반영되지는 않는다.

.. _export-import-host:

호스트 내보내기(Export Host) / 가져오기(Import Host)
======================================================
...
**파일명** — 기본값은 "export_hosts"이며 직접 수정할 수 있다. 확장자는 형식에 따라 자동으로 붙는다: JSON 형식은 ``.JSON``,
나머지 두 XML 형식은 ``.XML``.
