...
Load Database의 "Trigger" 파일 선택 미동작
============================================

Load Database 화면의 **"Trigger" 파일 선택 항목은 실제로 동작하지 않는다.** 화면에서 트리거 파일을 지정해도, CMS 쪽
코드에서 해당 처리 부분이 비활성화되어 있어(``#if 0``) 어떤 옵션도 ``loaddb`` 에 전달되지 않는다. 트리거는 이 화면으로
적재할 수 없다.
