...
로그인되지 않은 상태를 뜻한다. 로그인 방법은 :ref:`login-database` 참고.
...
호스트에 저장된 데이터베이스 로그인 프로필이 있으면 자물쇠 아이콘 옆에 열쇠 아이콘이 뜨고, 없으면 뜨지 않는다.
