**********************************************
부록: Next CUBRID Admin 서버 설치 및 설정
**********************************************

이 장은 Next CUBRID Admin 서버를 직접 설치·설정하는 관리자를 위한 부록이다. 이미 배포된 서버에 접속만 하는
...
.. important::
    ...
    ``SEED`` / ``SALT`` 는 저장된 호스트 접속 비밀번호나 로그인 프로필 같은 민감한 데이터를 암호화하는 데 쓰이는
    암호화 키를 만들기 위해 사용되는 값이다. 이 두 값이 있어야만 암호화된 데이터를 원래대로 복호화할 수 있다. ``cwm.conf`` 에 직접 넣어도 무시되며(실수로 값을 바꾸지 못하게 막는 안전장치), 최초
    실행 시 자동 생성되어 ``cwm-vault/secrets.json`` 파일에 저장된다.

    **이 파일을 삭제하거나 내용이 바뀌면 그동안 저장해 둔 모든 데이터를 다시는 복호화할 수 없게 된다** — 절대
    직접 편집하거나 삭제하지 않는다. 서버를 백업할 때는 ``cwm.conf`` 뿐 아니라 이 ``secrets.json`` 파일도
    함께 백업해야 한다.
