...
* 탭-스위처 아이콘 자체를 우클릭하면 "ALL BROKERS" 메뉴(Start All / Stop All / Restart All / **Edit Broker Config** /
  **Show Status** 등)가 나온다.
...
탭-스위처 아이콘 우클릭 → "ALL BROKERS" 메뉴 → **Show Status** 를 선택하면(또는 개별 브로커 우클릭 →
**Show Status** 로 그 브로커 하나만) 브로커 상태 탭이 열린다.

* 표 컬럼: **Name**, **Status**, PID, Port, AS(Application Server 수), JQ(Job Queue), REQ(요청 수), TPS, QPS,
...
* 우측 상단에 새로고침 버튼과 :doc:`dashboard` 문서에서 설명한 자동 새로고침 설정 팝오버가 있다 — 이 화면의
  자동 새로고침은 Dashboard/Broker 간격 설정 중 **Broker** 간격을 따른다. 간격이 0보다 크면 헤더에 LIVE
...
브로커 우클릭 → **Properties** 를 선택하면 열린다. 위 "Broker Config 편집"이 ``cubrid_broker.conf`` 파일
전체를 텍스트로 편집하는 화면인 것과 달리, 이 화면은 우클릭한 브로커 한 개의 파라미터만 표 형태로 보여주고
수정하는 구조화된 편집기이다. 저장 시에도 그 브로커의 섹션에만 값이 반영되고, 파일의 다른 브로커나 공통
``[broker]`` 섹션은 건드리지 않는다.

* **Common** / **Advanced** 두 탭에 파라미터가 나뉘어 있다(Common 11개, Advanced 16개, 총 27개). 목록은
...
* 값을 바꾼 파라미터는 표에서 강조 표시되고, 상단에 변경된 개수가 배지로 표시된다. 하나 이상 바뀌어야
  **Apply** 버튼이 활성화된다.
* **Reset** 으로 변경 사항을 되돌릴 수 있다(값을 하나라도 바꾼 뒤에만 나타난다).

.. warning::

    **Apply** 는 실제 ``cubrid_broker.conf``\ 에 해당 브로커 섹션 값만 즉시 반영한다. 신중하게 사용한다.
