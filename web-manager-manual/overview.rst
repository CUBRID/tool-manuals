...
이 매뉴얼 전반에서 필드 이름 뒤의 ``*`` 표시는 필수 입력 항목을 뜻한다.
...
* **프로필(저장된 로그인 정보)** — 호스트나 데이터베이스에 로그인할 때 "비밀번호 저장"을 켜두면, 다음부터 다시
  ...
  이렇게 저장된 로그인 정보를 편의상 "프로필"이라고 부른다(예: :doc:`host_operations` 의 저장된 호스트 접속 정보,
  :doc:`database_operations` 의 "데이터베이스 로그인 프로필"). NCA 계정으로 다시 로그인하면 프로필도 함께 불러와진다.
