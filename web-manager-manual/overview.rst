...
* **NCA** — Next CUBRID Admin의 약어. 이 매뉴얼이 다루는 웹 기반 관리 도구 자체를 가리키며, 이후 모든 장에서
  제품 이름 대신 NCA로 줄여 쓴다.
...
* **대화창(Dialog)** — 화면 위에 팝업으로 뜨는 창을 이 매뉴얼에서는 전부 대화창이라고 부른다(예: 호스트 추가
...
* **프로필(저장된 로그인 정보)** — 호스트나 데이터베이스에 로그인할 때 "비밀번호 저장"을 켜두면, 다음부터 다시
  입력하지 않도록 NCA 서버가 로그인 계정별 암호화된 파일에 그 사용자명/비밀번호를 저장해둔다. 이 매뉴얼에서는
  이렇게 저장된 로그인 정보를 편의상 "프로필"이라고 부른다(예: :doc:`host` 의 저장된 호스트 접속 정보,
  :doc:`database` 의 "저장된 로그인 프로필"). NCA 계정으로 다시 로그인하면 프로필도 함께 불러와진다.

이 매뉴얼에는 이름이 비슷해 혼동하기 쉬운 세 종류의 "계정"과 네 종류의 "계획/자동화"가 나온다. 각각 서로
