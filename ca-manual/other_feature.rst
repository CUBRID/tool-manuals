...
CUBRID ADMIN 클라이언트를 수행 중에 발생한 오류 로그는 $CUBRID/cubridmanager/workspace/logs에 cubrid.txt라는 파일로 생성된다.

사용자는 CUBRID ADMIN 개선 및 오류 분석 시 해당 오류 로그와 현상을 CUBRID ADMIN 개발 사이트 이슈로 등록하여 CUBRID ADMIN 개선 활동에 참여할 수 있다.

질의 수행 자동화
================

자동으로 주기적으로 질의를 수행하도록 설정할 수 있다.

질의 자동화 추가
-----------------

.. image:: /images/cm-add-auto-exec-query.png

.. rubric:: 좌측 탐색 창 > 데이터베이스 이름 펼침 > 하단에 작업 자동화 > 질의 자동화 > 질의 자동화 우클릭 > 질의 자동화 추가

.. image:: /images/cm-add-auto-exec-query-panel.png

*   질의 ID, DB 사용자 이름, 비밀번호를 입력합니다.
*   질의의 수행 주기를 설정합니다.
*   질의를 입력하고, 질의 검사를 통해 유효성을 검증받습니다.
*   확인 버튼을 눌러 질의 자동화를 추가합니다.

질의 자동화 삭제
---------------

.. rubric:: testdb > 작업 자동화 > 질의 자동화 > 대상 질의 자동화 우클릭 > 질의 자동화 삭제


질의 자동화 편집
---------------

.. rubric:: testdb > 작업 자동화 > 질의 자동화 > 대상 질의 자동화 우클릭 > 질의 자동화 편집
...
.. note:: 
    ...
    *   이 기능은 CUBRID에서 기본 지원되는 기능이 아니며, CUBRID ADMIN에서 자체 지원되는 기능이므로 한계가 있을 수 있다. 
    *   처음 사용 시 테이블/칼럼 설명을 저장하는 별도의 테이블이 생성되어야 사용할 수 있는 기능이며, DBA 권한이 있을 경우에만 이 테이블을 생성할 수 있다.

DBA 사용자일 경우 아래와 같이 안내 후 _cub_schema_comments 테이블을 자동 생성한다. 탐색기 창에서는 일반 사용자들이 이 테이블을 UI에서 편집하지 못하도록 시스템 테이블 영역에 출력이 되지만 실제 시스템 테이블은 아니다.

.. image:: /images/cm-col-comment.png

DBA 권한이 없는 사용자의 경우 아래와 같은 오류가 발생한다.
...
3)  CUBRID ADMIN에서 각 마스터/슬레이브 DB에 로그인한다. 
...
여기까지 해서 간단한 HA 환경 구성이 완료되었다. 보다 자세한 사항은 CUBRID 매뉴얼의 HA 설정 부분을 참고한다.
