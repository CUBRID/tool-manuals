...
| 구분 | 표기 | 현재 본문 |
|:---|:---|:---:|
| **기준 표기** | `CUBRID Admin` | 0건 |
| 약어 | `CA` | |
| 금지 표기 | `CUBRID ADMIN` | **127건** |
| 금지 표기 | 큐브리드 어드민, CUBRID 관리자 도구, CUBRID Administrator, CUBRID admin | 0건 |

새로 작성하는 문장은 `CUBRID Admin`을 사용합니다.
사람 역할을 뜻하는 `관리자`, `DBA`는 일반명사이므로 그대로 사용합니다.

> **기존 본문 127건은 전부 금지 표기(`CUBRID ADMIN`)입니다.**
> 저장소 `README.md`가 `CUBRID Admin`으로 표기하므로 기준을 그쪽으로 확정했습니다.
> 127건의 전수 수정은 **별도 일괄 수정 PR**로 처리합니다.
> 이 PR에서 변경되지 않은 줄은 지적하지 않으므로 기존 127건은 리뷰 대상이 아닙니다.
> 단 작성자가 `CUBRID ADMIN`이 포함된 줄을 수정하면 그때 함께 `CUBRID Admin`으로 고칩니다.
