...
.. image:: /images/cm-host-edit-menu.png

.. rubric:: 좌측 호스트 탐색 창 > 그룹 이름 펼침 > 호스트 이름 더블 클릭

.. image:: /images/cm-host-edit-window.png

CUBRID ADMIN를 설치 후 처음 실행하게 되면 호스트 탐색 트리에 "localhost"라는 호스트가 기본으로 설정되어 있다. 현재 CUBRID ADMIN 클라이언트가 실행되는 PC에 구동 중인 CUBRID ADMIN 서버가 있다는 가정 아래 설정된 것이며, CUBRID ADMIN 접속에 대한 기본 정보를 알려주기 위해 제공된다.

호스트 이름을 더블클릭하면 편집 창이 팝업되는데, 편집할 호스트 정보는 아래와 같다.

*   호스트 이름: 호스트 이름은 CUBRID ADMIN 내에서 관리하고자 하는 호스트를 구분하는 구분자로, 유일한 이름을 부여한다. 내부적으로 관리하는 호스트 구분자는 "호스트 주소와 연결 포트"이다. 호스트 이름으로는 숫자와 알파벳만 입력할 수 있으며, 공백을 포함할 수 없다. 호스트 이름의 길이는 최소 4자리 최대 32자리이다.
...
*   사용자 이름: CUBRID ADMIN의 관리자 권한을 가진 사용자는 admin이며, 설치 후 처음 사용할 때에는 반드시 admin 계정으로 접속해야 한다. 사용자 이름으로는 숫자와 알파벳만 입력할 수 있으며, 공백을 포함할 수 없다. 사용자 이름의 길이는 최소 4자리 최대 32자리이다.

*   비밀번호: CUBRID ADMIN의 관리자 권한을 가진 admin 사용자의 최초 비밀번호는 "admin"이다. admin 사용자가 최초 연결할 때 비밀번호를 재설정하지 않으면 접속할 수 없다. 이때 재설정한 admin 사용자의 비밀번호는 분실하지 않도록 주의한다. 비밀번호는 "admin"으로 재설정할 수 없다. 비밀번호는 공백을 포함할 수 없으며, 비밀번호는 길이는 최소 4자리 최대 32자리이다.

*   비밀번호 저장: 비밀번호를 로컬에 저장한다. 선택하면 더블클릭해서 자동으로 이 호스트에 로그인할 수 있다. 선택하지 않으면 비밀번호를 입력하는 대화상자가 나타난다.

*   드라이버 버전: CUBRID ADMIN에서 사용할 CUBRID JDBC 드라이버를 선택할 수 있다. 기본값은 "Auto Detect"로, 먼저 서버의 버전을 확인한 후 CUBRID ADMIN에 등록된 JDBC 드라이버 중 동일한 버전이 있는가를 확인하여 자동으로 연결한다. 연결할 수 있는 JDBC 드라이버가 없으면 다음과 같이 JDBC를 선택하여 등록하라는 확인 메시지가 출력된다. CUBRID JDBC 드라이버는 CUBRID 홈페이지에서 다운로드 받을 수 있다.
...
호스트 추가
-----------
.. image:: /images/cm-host-add-menu.png

.. rubric:: 좌측 호스트 탐색 창 > 호스트 그룹에서 마우스 우클릭 > 호스트 추가
...
.. image:: /images/cm-host-add-window.png
...
호스트 연결에 사용할 JDBC 드라이버는 드라이버 버전 란에서서 찾아보기를 통해 추가할 수 있다.

*   CUBRID JDBC 드라이버는 CUBRID 홈페이지에서 다운로드 받을 수 있다.
*   HA로 구성된 호스트의 경우, 마스터는 [마스터], 슬레이브는 [슬레이브] 가 표시된다.
...
호스트 탐색 트리에 등록된 호스트 정보를 삭제한다. 호스트 삭제 시 하부 노드의 모든 정보가 같이 삭제된다. 단, CUBRID ADMIN의 클라이언트 구성 정보만 삭제되는 것으로, CUBRID ADMIN 서버에 구성된 정보까지 삭제되는 것은 아니다.
...
*   호스트 정보 가져오기: "호스트 정보 내보내기..."를 이용하여 저장한 xml 파일을 가져와서 사용중인 CUBRID ADMIN에 추가한다. 단, 동일한 이름의 호스트 정보는 가져올 수 없다.
...
CUBRID ADMIN은 워크스페이스 개념으로 호스트 정보, 설정 정보를 관리한다. 신 버전을 별도의 경로에 설치할 경우 다음 2가지 방법으로 기존 정보를 마이그레이션할 수 있다.
신 버전 CUBRID ADMIN 설치후 워크스페이스 선택시 기존 버전이 설치된 폴더 내부에 있는 workspace를 선택하거나 복사한 후 선택한다
신 버전 CUBRID ADMIN 설치후 워크스페이스 선택시 기본 값을 사용하고 "파일 > 워크스페이스 가져오기..."를 이용하여 가져온다. 만약, 오래된 버전의 CUBRID ADMIN의 사용 환경을 이용하려면 "워크스페이스 가져오기"를 이용해야 한다.

서비스 관리하기
===============

CUBRID ADMIN로 CUBRID 관련 서비스를 시작/정지할 수 있다. 이것은 CUBRID 데이터베이스 명령어인 cubrid 명령과 동일한 기능이다. 단, CUBRID ADMIN 서버는 CUBRID ADMIN 클라이언트를 통해서 구동/정지할 수 없다.
...
.. rubric:: 좌측 호스트 탐색 창 > 호스트 이름(마우스 우클릭) > 속성 > 파라미터 구성 > 서비스 구동 설정

호스트 이름에 대고 마우스 우버튼을 클릭하고 "속성"을 선택하여 "서비스 구동 설정"에서 시작할 서비스와 데이터베이스를 선택한다. 시작할 서비스에는 server, broker, manager가 있는데, manager의 선택 여부는 비활성화 되어 있다. (CUBRID ADMIN에서 관리 기능을 사용하려면 매니저 서버를 구동한 상태여야 하기 때문이다.)
...
.. rubric:: 좌측 호스트 탐색 창 > 브로커 이름에서 마우스 우클릭 > 브로커 시작/정지/재시작
...
CUBRID 서버를 설치 후 CUBRID ADMIN로 접속하면 demodb가 기본으로 존재하는 것을 볼 수 있다. 이 장에서는 데이터베이스 생성, 시작/정지, 백업/복구, 유지보수, 정보 조회 등 데이터베이스 관리 방법에 대해 설명한다.
...
.. image:: /images/cm-database-backup.png

최초 백업이기 때문에 백업 수준은 Level0만 볼 수 있다. "확인"을 눌러 현재 상태로 백업을 한다.
백업 디렉터리가 없을 경우 백업 디렉터리 생성 대화창이 출력되는데 "확인"을 클릭하면 계속 진행할 수 있다.

참고로, 방금 실행한 Level0 백업은 전체 백업(full backup)에 속한다. 백업 옵션과 관련된 상세 정보는 CUBRID 매뉴얼을 참고한다. 

.. rubric:: 앞서 팝업된 데이터베이스 백업 창 > 백업 정보 탭 또는 백업 수행 이력 탭

.. image:: /images/cm-database-backup-record.png
...
CUBRID ADMIN 서버를 통해서 주기적으로 백업을 수행하고자 하는 경우 백업 자동화 기능을 사용하며, "백업 자동화 추가" 메뉴에서 관련 정보를 설정한다. 
...
*   온라인 백업: 데이터베이스가 구동 중인 상태에서만 자동 백업을 실행한다. 중지 상태에서는 데이터베이스를 백업하지 않고 오류 로그만 기록한다.
...
복구 과정을 살펴보기 위해, 먼저 테이블을 임의로 삭제한 후 앞에서 생성한 백업 파일로 testdb를 복구해보자.
...
testdb에 데이터베이스를 로드하는 방법은 다음과 같다. 이를 위해 우선 DB에 로그인하고 DB를 정지시켜야 한다.

.. rubric:: testdb 우클릭 > 데이터베이스 정지

데이터베이스를 정지했다면 다음과 같은 순서로 데이터베이스를 로드한다.

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 로드

.. image:: /images/cm-database-load.png

*   기존에 언로드된 정보를 목록에서 선택하거나, 하단의 "원격 시스템에서 언로드된 파일 선택"을 체크한다.
*   로드할 정보를 선택한다.
*   로드 옵션을 선택하고 "확인"을 눌러 데이터베이스 로드를 진행한다.

데이터베이스 언로드
^^^^^^^^^^^^^^^^^^

testdb에 데이터베이스를 언로드하는 방법은 다음과 같다. 이를 위해 우선 DB에 로그인해야 한다.

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 언로드

.. image:: /images/cm-database-unload.png

*   데이터베이스를 언로드할 경로를 설정한다.
*   언로드 대상을 선택한다.
*   언로드 옵션을 선택한다.
*   "확인" 버튼을 눌러 언로드를 진행한다.
...
testdb의 일관성을 검사하는 방법은 다음과 같다. 

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 검사

.. image:: /images/cm-database-check.png

*   필요시 "불일치 정보 발견시 복구" 옵션을 사용해 복구할 수 있다.


데이터베이스 최적화
^^^^^^^^^^^^^^^^^^

testdb를 최적화하는 방법은 다음과 같다. 이를 위해서는 우선 testdb를 정지시켜야 한다.

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 최적화

.. image:: /images/cm-database-optimize.png

최적화할 테이블을 선택하고 "확인"을 눌러 진행한다.

데이터베이스 공간 검사
^^^^^^^^^^^^^^^^^^^^^^

testdb의 공간을 검사하고 정리하는 방법은 다음과 같다. 이를 위해서는 우선 testdb를 정지시켜야 한다.

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 공간 정리

.. image:: /images/cm-database-space-optimize.png

데이터베이스 이름 변경
^^^^^^^^^^^^^^^^^^^^^^

testdb의 이름을 변경하는 방법은 다음과 같다. 이를 위해서는 우선 testdb를 정지시켜야 한다.

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 이름 변경

.. image:: /images/cm-database-change-name.png

데이터베이스 복사
^^^^^^^^^^^^^^^^

testdb를 복사하는 방법은 다음과 같다. 이를 위해서는 우선 testdb를 정지시켜야 한다.

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 복사

.. image:: /images/cm-database-copy.png

*   "대상 데이터베이스" 섹션에서 데이터베이스 이름을 입력하여 복사된 데이터베이스의 이름을 설정한다.
*   "개별 볼륨 복사 설정"을 통해 개별 볼륨을 복사할 수 있다.
*   "동일한 파일 덮어 쓰기", "원본 데이터베이스 삭제"와 같은 옵션을 체크박스를 통해 설정할 수 있다.

데이터베이스 삭제
^^^^^^^^^^^^^^^^

데이터베이스를 삭제하는 방법은 다음과 같다. 데이터베이스가 정지되지 않았다면 정지한다.
...
testdb에 적용된 잠금(락)에 대한 정보는 다음과 같이 확인할 수 있다.

.. rubric:: testdb 우클릭 > 데이터베이스 정보 > 데이터베이스 잠금 정보

데이터베이스 잠금 정보 화면에서는 현재 데이터베이스에 설정된 모든 잠금 정보를 확인할 수 있다.

데이터베이스 트랜잭션 정보
^^^^^^^^^^^^^^^^^^^^^^^^^^

testdb에 적용된 트랜잭션에 대한 정보는 다음과 같이 확인할 수 있다.

.. rubric:: testdb 우클릭 > 데이터베이스 정보 > 데이터베이스 트랜잭션 정보

데이터베이스 트랜잭션 정보 화면에서는 현재 실행 중인 트랜잭션의 상세 정보를 확인할 수 있다.

OID 탐색기
^^^^^^^^^^

테이블 내의 데이터의 OID를 통해 데이터를 조회할 수 있다.

.. rubric:: testdb 우클릭 > 데이터베이스 정보 > OID 탐색기

.. image:: /images/cm-oid-navigator.png
...
testdb의 속성을 통해 연결정보와 서버 고유 변수를 설정할 수 있다.

.. rubric:: testdb 우클릭 > 속성


.. image:: /images/cm-database-properties-connection.png
.. image:: /images/cm-database-properties-server-variable.png

*   각 프로퍼티 요소에 대한 설명은 CUBRID 매뉴얼을 참조하도록 한다.

스키마 관리하기
===============

CUBRID ADMIN은 데이터베이스 스키마를 관리하고 분석하는 기능을 제공한다.
...
CUBRID ADMIN은 데이터베이스의 모든 테이블 명세를 엑셀로 출력하는 기능을 제공한다. 테이블 목록과 각 테이블별 상세 스키마 정보를 표로 제공하며, 테이블 칼럼/설명 기능을 이미 사용하고 있다면 이미 입력된 설명과 함께 출력된다.
...
두 개 이상의 데이터베이스를 선택하여 데이터베이스 스키마를 비교할 수 있다. 이 기능은 단순히 차이를 비교하는 것을 넘어 각 테이블별 칼럼, PK, 인덱스 등 상세 분석과 레코드 개수까지 동시에 비교할 수 있다. 

HA로 구성된 서버의 마스터와 슬레이브 노드를 비교하는 용도로도 사용될 수 있다.

.. image:: /images/cm-database_compare.png

CUBRID ADMIN 사용자 관리하기
=============================
...
매니저 사용자 정보는 CUBRID ADMIN 서버에서 저장되며, 해당 호스트에 접속하기 위해서는 반드시 사전에 CUBRID ADMIN 사용 권한을 취득해야 한다.

CUBRID ADMIN에는 관리자 계정인 admin과, admin이 등록한 일반 사용자만 접속할 수 있다. 그리고 CUBRID ADMIN 사용자 관리 기능은 admin에게만 허용된다.
...
CM 관리자 편집 창에서 추가 버튼을 클릭하면 사용자 추가 창이 팝업되고, 이 창에서 CUBRID ADMIN 사용자 계정 정보 및 권한 설정과 데이터베이스 접속 권한 설정을 할 수 있다.


CUBRID ADMIN 사용 계정 이름 및 권한 설정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
CUBRID ADMIN 사용자에게 데이터베이스 권한 설정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
*   연결 권한: 추가 중인 CUBRID ADMIN 사용자가 연결할 수 있는 데이터베이스를 선택한다. "Yes"를 선택한 데이터베이스만 호스트 탐색 트리에 출력된다.
*   DB 계정: 추가 중인 CUBRID ADMIN 사용자가 해당 데이터베이스에 접속할 때 사용하는 데이터베이스 계정 정보를 입력한다. "dba" 또는 "public"과 같은 값을 입력할 수 있다.
*   브로커 주소: 해당 데이터베이스에 접속하기 위해 사용할 브로커 주소를 입력한다. 기본값은 데이터베이스 서버의 주소와 동일하다. 브로커 서버가 분리되어 있다면 이 값을 변경하여 접속 정보를 부여할 수 있다.
*   브로커 포트: 추가 중인 CUBRID ADMIN 사용자가 해당 데이터베이스에 접속할 때 사용할 브로커 포트를 정의한다. 브로커 포트는 현재 정의되어 있는 브로커 정보를 통해서 선택할 수 있다. 브로커 정보는 브로커 이름/포트/구동상태로 구성되어 있다.
...
편집할 CUBRID ADMIN 사용자를 목록에서 선택한 후, 사용자 추가와 동일한 방법으로 사용자를 편집할 수 있다. 단, admin 계정에 대한 편집은 비밀번호만 변경할 수 있으며, 나머지 권한은 admin 권한에서 다른 권한으로 변경할 수 없다.

CUBRID ADMIN 사용자 삭제
^^^^^^^^^^^^^^^^^^^^^^^^^

.. rubric:: 도구 > CM 관리자 편집 > 삭제 버튼

삭제할 CUBRID ADMIN 사용자를 목록에서 선택한 후 "삭제" 버튼을 클릭하면 사용자를 삭제할 수 있다. 단, admin 계정은 삭제할 수 없다.
...
CUBRID ADMIN은 스키마, 데이터를 파일로부터 데이터베이스 서버로 가져오기(import)를 지원한다. 스키마는 SQL 파일만 지원하며, 데이터는 SQL, CSV, XLS, TXT를 지원한다.
...
.. note::
    ...
    XLSX 파일은 엑셀 2007부터 추가된 파일 포맷이며 CUBRID ADMIN은 XLSX 파일을 지원하지 않으므로, XLSX 파일을 원본으로 하여 데이터 가져오기 작업을 수행하는 경우 정상적인 데이터 입력을 보장할 수 없다. 따라서, XLSX 파일은 "다른 이름으로 저장" 메뉴를 통해 반드시 XLS 파일로 변환한 후에 사용하도록 한다.
...
CUBRID ADMIN은 스키마, 데이터를 데이터베이스 서버에서 파일로 내보내기(export)를 지원한다. 스키마는 SQL 파일만 지원하며, 데이터는 SQL, CSV, XLS, TXT를 지원한다.
...
서비스가 복잡해지고 데이터베이스 입출력이 많아지면 브로커 또한 목적에 맞게 분리하여 사용하게 된다. 이렇게 브로커가 여러 개로 분리되는 경우 여러 브로커의 설정을 하나씩 설정하기가 어려우므로, CUBRID ADMIN은 특정 서버에 있는 모든 브로커 설정을 한 화면에서 비교하면서 편집하는 기능을 제공한다.
...
해당 브로커의 SQL_LOG 파라미터 값이 ON이면 해당 브로커를 통해서 수행된 모든 질의는 로그 파일에 기록된다. 사용자는 이 로그 파일을 CUBRID ADMIN에서 보고 분석하거나 재실행해 볼 수 있다.
...
*   분석 결과: 로그 분석 결과를 보여준다. "트랜잭션 기반 분석"을 선택했다면 각 트랜잭션의 수행 시간이 표시되고, 선택하지 않았다면 각 질의별 분석 정보(전체 수행 횟수, 오류 횟수, 최고 수행 시간, 최저 수행 시간, 평균 수행 시간)가 표시된다. 결과 분석 목록의 칼럼을 클릭하면, 선택된 칼럼 값을 기준으로 분석 결과가 정렬된다.
...
접근 로그(access log) 파일은 응용 클라이언트 접속에 관한 정보를 기록하며, "broker_name.access"의 이름으로 저장된 것을 분석하여 출력한다. 또한, 브로커 환경 설정 파일에서 LOG_BACKUP 파라미터가 "ON"으로 설정된 경우, 브로커의 구동이 정상적으로 종료되면 접속 로그 파일에 종료된 날짜와 시간 정보가 추가되어 로그 파일이 저장된다.
