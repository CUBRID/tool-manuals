...
CUBRID 서버를 설치 후 CUBRID 매니저로 접속하면 demodb가 기본으로 존재하는 것을 볼 수 있다. 이 장에서는 데이터베이스 생성, 시작/정지, 백업/복구, 유지보수, 정보 조회 등 데이터베이스 관리 방법에 대해 설명한다.
...
CUBRID 매니저는 데이터베이스 스키마를 관리하고 분석하는 기능을 제공한다.
...
CUBRID 매니저는 데이터베이스의 모든 테이블 명세를 엑셀로 출력하는 기능을 제공한다. 테이블 목록과 각 테이블별 상세 스키마 정보를 표로 제공하며, 테이블 칼럼/설명 기능을 이미 사용하고 있다면 이미 입력된 설명과 함께 출력된다.
