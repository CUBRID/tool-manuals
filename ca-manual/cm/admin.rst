...
tesbdb를 최적화하는 방법은 다음과 같다. 이를 위해서는 우선 testdb를를 정지시켜야한다.

.. rubric:: tesbdb 우클릭 > 데이터베이스 관리 > 데이터베이스 최적화

.. image:: /images/cm-database-optimize.png

이후 최적화할 테이블을 선택하고 확인을 눌러 진행한다.

데이터베이스 공간 검사
---------------------

testdb의 공간 정리 방법은 다음과 같다. 이를 위해서는 우선 testdb를 정지시켜야한다.

.. rubric:: tesbdb 우클릭 > 데이터베이스 관리 > 데이터베이스 공간 정리
...
.. rubric:: tesbdb 우클릭 > 데이터베이스 관리 > 데이터베이스 이름 변경
...
testdb의 복사 방법은 다음과 같다. 이를 위해서는 우선 testdb를 정지시켜야한다.

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 복사

.. image:: /images/cm-database-copy.png

*   대상 데이터베이스 섹션 내부의 데이터베이스 이름을 입력하여 복사된 데이터베이스가 가질 이름을 설정한다.
*   개별 볼륨 복사 설정을 통해 개별 볼륨을 복사할 수 있다.
*   동일한 파일 덮어 쓰기, 원본 데이터베이스 삭제와 같은 옵션을 체크박스를 통해 설정할 수 있다.
