...
팝업된 창에서 "확인" 버튼을 누르면 해당 데이터베이스에서 사용하는 매개 변수(parameter) 정보를 출력하며, 콘솔 창에서 "cubrid paramdump" 명령을 실행해도 같은 값을 확인할 수 있다. "cubrid paramdump"에 관한 자세한 내용은 CUBRID 매뉴얼의 큐브리드 메뉴얼의 paramdump 부분을 참고한다.
...
데이터베이스 서버 파라미터 값을 설정한다. 자세한 것은 큐브리드 메뉴얼을 참고한다.
...
설정 파일에 대한 자세한 정보는 큐브리드 메뉴얼을 참고한다.
