...
팝업된 창에서 "확인" 버튼을 누르면 해당 데이터베이스에서 사용하는 매개 변수(parameter) 정보를 출력하며, 콘솔 창에서 "cubrid paramdump" 명령을 실행해도 같은 값을 확인할 수 있다. "cubrid paramdump"에 관한 자세한 내용은 CUBRID 매뉴얼의 paramdump 부분을 참고한다.

팝업된 창에 있는 "서버/클라이언트 매개 변수 정보 출력" 체크 박스를 체크하고 "확인" 버튼을 누르면 서버에 적용된 파라미터 값과 클라이언트에 적용된 파라미터 값을 모두 확인할 수 있고, 선택을 해제하면 서버에 적용된 파라미터 값만 확인할 수 있다. 서버와 클라이언트의 적용 구분에 대한 자세한 설명은 큐브리드 설정 부분을 참고한다.
...
"cubrid paramdump"에 관한 자세한 내용은 CUBRID 매뉴얼의 plandump 부분을 참고한다.
...
데이터베이스 서버 파라미터 값을 설정한다. 자세한 것은 큐브리드 매뉴얼의 큐브리드 설정 부분을 참고한다.
...
브로커 파라미터 값을 설정한다. 자세한 것은 큐브리드 매뉴얼의 브로커 설정 부분분을 참고한다.
...
설정 파일에 대한 자세한 정보는 큐브리드 매뉴얼을을 참고한다.
