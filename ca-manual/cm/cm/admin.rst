...
CUBRID 서버를 설치후 CUBRID ADMIN로 접속을 하면 demodb가 기본으로 존재하는 것을 볼 수 있다. 이 장에서는 예를 통해 데이터베이스를 새로 생성하고 삭제하는 방법에 대해 알아본다.
...
testdb에 데이터베이스를 로드하는 방법은 다음과 같다. 이를 위해 우선 DB에 로그인하고 DB를 정지시켜야한다.

.. rubric:: testdb 우클리 > 데이터베이스 정지

db를 정지했다면 다음과 같은 순서로 데이터베이스를 load한다.

.. rubric:: testdb 우클릭 > 데이터베이스 관리 > 데이터베이스 로드드 

*   기존에 언로드된 정보를 리스트에서 선택하거나, 하단의 원격 시스템에서 언로드된 파일 선택을 체크한다.
*   로드할 정보를 선택한다.
*   이후 로드 옵션을 선택하고 확인을 눌러 데이터베이스 로드를 진행한다.

데이터베이스 언로드
-------------------

testdb에 데이터베이스를 언로드하는 방법은 다음과 같다. 이를 위해 우선 DB에 로그인한다.

.. rubric:: tesbdb 우클릭 > 데이터베이스 관리 > 데이터베이스 언로드
...
.. rubric:: testdb > 데이터베이스 정보 > OID 탐색기
